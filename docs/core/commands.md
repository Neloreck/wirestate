...
`execute` throws `WirestateError` when no handler is registered. `executeAsync` always returns a Promise, so the same
miss arrives as a rejection instead: await it or attach a `.catch`, since a `try` around the call alone will not see it.
