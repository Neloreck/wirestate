...
export abstract class AbstractService {
   ...
   */
  protected getInitialState<T extends TAnyObject>(
    ServiceClass?: TInitialStateKey,
  ): T | null {
    const initialState: InitialState =
      this.getContainer().get<InitialState>(INITIAL_STATE_TOKEN);

    return (
      (ServiceClass
        ? initialState.getFor<T>(ServiceClass)
