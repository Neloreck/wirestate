...
}function bindService(container, token, ServiceClass, isOnceBind, isIgnoreLifecycle) {
...
function _detachQueryUnregs(service) {
  ...
  if (container.isBound(INITIAL_STATE_TOKEN)) {
    ...
    container.rebind(INITIAL_STATE_SHARED_TOKEN).toConstantValue(next.getShared());
  } else {
    container.bind(INITIAL_STATE_TOKEN).toConstantValue(next);
    container.bind(INITIAL_STATE_SHARED_TOKEN).toConstantValue(next.getShared());
  }
}const IocContext = createContext(null);
IocContext.displayName = "IocContext";function createServicesProvider(services, options = {}) {
    const { activate } = options;
    if (activate && activate.length > 0) {
        for (const eager of activate) {
            if (!services.includes(eager)) {
                throw new Error(`[ioc] createServicesProvider: '${eager.name}' is listed in 'activate' but was not provided in 'services'.`);
            }
        }
    }
  ...
  return useCallback((type, data) => container.get(QUERY_BUS_TOKEN).query(type, data), [container]);
...
const ERROR_CODE_ACCESS_BEFORE_ACTIVATION = 2;class WirestateError extends Error {
  name = "WirestateError";
  ...
  return token ? bindService(container, token, ServiceClass) : bindService(container, ServiceClass, ServiceClass);
