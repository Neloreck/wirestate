...
`;
  ...
  it("should transform a service module and derive a root relative id", () => {
    ...
    expect(result?.code).toContain('openHotModule("src/services/counter.ts")');
    expect(result?.code).toContain("__wirestate_hot__.closeHotModule()");
  });
  ...
  it("should not strip a sibling directory that only shares the root prefix", () => {
    ...
    expect(sibling?.code).toContain('openHotModule("/project-other/service.ts")');
    expect(insideRoot?.code).toContain('openHotModule("-other/service.ts")');
  });
  ...
  it("should ignore query suffixes appended by other plugins", () => {
    ...
    expect(result?.code).toContain('openHotModule("src/counter.ts")');
  });
