...
import { getInjectableTargets, isInjectable } from "./decorators";
import { Factory } from "./factory";
import * as Guards from "./providers";
import type { Provider } from "./providers";
import { type Token, isClassToken, toString, isInjectionToken, getToken } from "./tokens";
import { assertPresent, assertSingle, getParentClasses, promiseTry, windowedSlice } from "./utils";
...
export class Container {
  ...
  public getAsync<T>(token: Token<T>, options: { multi: true }): Promise<Array<T>>;
  public getAsync<T>(token: Token<T>, options: { optional: true }): Promise<T | undefined>;
  public getAsync<T>(token: Token<T>, options: { multi: true; optional: true }): Promise<Array<T> | undefined>;
  public getAsync<T>(token: Token<T>, options: { lazy: true }): () => Promise<T>;
  public getAsync<T>(token: Token<T>, options: { lazy: true; multi: true }): () => Promise<Array<T>>;
...
interface SingletonMap extends Map<Token<unknown>, Array<unknown>> {
  get<T>(token: Token<T>): Array<T> | undefined;

  set<T>(token: Token<T>, value: Array<T>): this;
}
