...
- **Lifecycle hooks**: class, value, and factory providers accept `onActivated(instance, container)` (may return a replacement value) and `onDeactivated(instance, container)`. Deactivation runs for container-owned (singleton-cached) values when their provider is unbound — in creation order on `unbindAll()`, with all bindings staying resolvable until every deactivation handler has run, so deactivating services can still communicate. Transient values are not tracked and never deactivate.
