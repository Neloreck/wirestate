...
- **Binding scopes**: factory descriptors accept `scope: "Singleton" | "Transient"` (`Singleton` remains the default). Transient factories construct a new value on every resolution and never cache. Value and instance descriptors are always singletons — transient class instances would bypass deactivation tracking, so they are intentionally unsupported.
