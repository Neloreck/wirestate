...
describe("useQueryExecutorAsync", () => {
  it("should return an executor that dispatches sync queries as promises", async () => {
    ...
    let executor: QueryExecutorAsync = null as unknown as QueryExecutorAsync;
    ...
    const result: string = await (executor as QueryExecutorAsync)("TEST_QUERY", "some-payload");
  ...
  it("should forward async handler results", async () => {
    ...
    const result: string = await (executor as QueryExecutorAsync)("ASYNC_QUERY", "value");
  ...
  it("should return a stable executor between re-renders", () => {
    const container: Container = new Container({ bindings: [QueryBus] });
    const executors: Array<QueryExecutorAsync> = [];

    function TestComponent() {
      executors.push(useQueryExecutorAsync());
